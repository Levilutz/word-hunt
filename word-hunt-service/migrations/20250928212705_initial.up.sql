...
CREATE TABLE IF NOT EXISTS versus_games_match_queue(
    id UUID PRIMARY KEY,
    queued_player_session_id UUID NOT NULL,
...
CREATE TABLE IF NOT EXISTS versus_games(
    ...
    player_a_session_id UUID NOT NULL,
    player_a_start TIMESTAMP,
    player_a_done BOOLEAN NOT NULL DEFAULT FALSE,
    player_b_session_id UUID NOT NULL,
    player_b_start TIMESTAMP,
    player_b_done BOOLEAN NOT NULL DEFAULT FALSE,
    grid JSONB NOT NULL
);

CREATE INDEX versus_games_created_at ON versus_games (created_at);
CREATE INDEX versus_games_player_a_session_id ON versus_games (player_a_session_id);
CREATE INDEX versus_games_player_b_session_id ON versus_games (player_b_session_id);

CREATE TABLE IF NOT EXISTS versus_game_submitted_words(
    id UUID PRIMARY KEY,
    game_id UUID NOT NULL REFERENCES versus_games(id) ON DELETE CASCADE,
    by_session_id UUID NOT NULL,
...
INSERT INTO versus_games (
    id, player_a_session_id, player_a_done, player_b_session_id, player_b_done, grid
)
VALUES (
...
INSERT INTO versus_game_submitted_words (id, game_id, by_session_id, tile_path, word)
VALUES
